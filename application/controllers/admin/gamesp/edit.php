...
class editController extends Controller {
	public function index($gameid = null) {
		...
		if($error) {
			...
			$this->response->redirect($this->config->url . 'admin/gamesp/index');
		}
		
		$game = $this->gamespModel->getGameById($gameid);
		...
		return $this->load->view('admin/gamesp/edit', $this->data);
	}
	...
	public function ajax($gameid = null) {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				...
				$this->gamespModel->updateGame($gameid, $gameData);
	...
	public function delete($gameid = null) {
		...
		$this->document->setActiveItem('gamesp');
		...
		$this->gamespModel->deleteGame($gameid);
	...
	private function validate($gameid) {
		...
		if(!$this->gamespModel->getTotalgamesp(array('game_id' => (int)$gameid))) {
