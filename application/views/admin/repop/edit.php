			...
			<div class="card card-custom">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Редактирование файла в репозитории PREMIUM
						</h3>
					</div>
				</div>
				<div class="card-body">
					<form id="editForm" method="POST">
						...
						<div class="m-btn-group m-btn-group--pill btn-group m-btn-group m-btn-group--pill btn-block" role="group" aria-label="Large button group">
							...
							<a data-toggle="tooltip" data-placement="right" title="" data-original-title="Удалить" style="height: 3.1rem;" href="/admin/repop/edit/delete/<?php echo $repo['repo_id'] ?>" class="btn btn-danger btn-elevate btn-icon"><i class="fa fa-trash-alt"></i></a>
						</div>
					</form>
				</div>
			</div>
...
<script>
	$('#editForm').ajaxForm({ 
		url: '/admin/repop/edit/ajax/<?php echo $repo['repo_id'] ?>',
		dataType: 'text',
		success: function(data) {
			...
			switch(data.status) {
				...
				case 'success':
					...
					setTimeout("redirect('/admin/repop')", 1500);
					break;
			}
		},
