...
class invoicesModel extends Model {
	...
	//Статистика
	public function getStatisticsInvStatsById() {
        $sql = "SELECT invoice_date_add, SUM(`invoice_ammount`) FROM invoices WHERE `invoice_status` = 1 GROUP BY `invoice_date_add`";
        $query = $this->db->query($sql);
        return $query->rows;
    }
}
